...
3. ✅ **DONE** (`795a209`) StageDirector reads `stage_config` (default `StageOneConfig.build()`). Removed the 5 `_get_wave_*` fns + WAVE_*_START_TIME + archetype preloads + ELITE_AFFIX + the 2 hardcoded elites; wave selection = `get_active_wave`, elites iterate `elite_events`, demon-seal/boss/duration from config. `_apply_stage_config_values()` populates the boss_*/demon_seal_* fields (minimal churn). `pool.assign()` handles Array[EnemyArchetype]→Array[Resource]. All 10 signals + spawn loop + OQ-4 guard byte-identical. CI 110 tests green (incl. the StageDirector-instantiating demon-seal-guard test). ⚠️ **STILL NEEDS Stage-1 playtest** — CI validated parse/type + config values, but the live apply_wave_config integration is the one thing CI can't run.
4. ⏳ **NEXT — RunDirector. ⚠️ HIGH RISK + UNTESTABLE BY CI**: moving Player-spawn out of CharacterSelectPanel + pause/GameOverPanel out of HUD + Main.tscn restructure is invasive runtime SCENE wiring — a mistake breaks game launch, and NEITHER CI nor unit tests catch scene-wiring/launch failures (no local Godot to run the game). This step's correctness can ONLY be confirmed by the user running the game. Recommend: playtest Stage 1 (verify Step 3) BEFORE building RunDirector, so a Step-4 launch break isn't conflated with a Step-3 regression. Do Step 4 in small reversible commits + playtest each.
