...
6b. ⏳ **NEXT (headless-testable, but touches playtested code)**: BossBase refactor — extract `FamineBeastBoss` → `BossBase` (shared BossState/cooldowns/enrage/summon-cap/telegraph/victory) + `FamineBeastBoss` + `GhostMarketJudge` subclasses (boss-system.md r2: 勾魂锁链/判笔/生死簿召唤/审判终结). ⚠️ MUST keep FamineBeastBoss byte-identical (Stage 1 boss is playtested) → eventual Stage-1 boss playtest. Ability cooldown/enrage LOGIC is unit-testable.
