...
> **Status**: Approved (revision-2 — second /design-review verdict on revision-1 was CONCERNS with 1 new RECOMMENDED + 1 NICE-TO-HAVE; revision-2 closes both. revision-1 closed all 8 original revision-0 findings.)
> **Author**: claude (revision-2 by claude — narrow scope: AC-12 + Edge Case code-truth correction for DEFEATED XP-credit behavior + Combat GDD revision log row order fix)
> **Last Updated**: 2026-05-25 (revision-2, approved pending fourth review)
...
- **If `gain_experience` is called while `DEFEATED`** (code-true behavior): the call **early-returns** before any mutation (`player.gd:140-141`: `if _is_dead or amount <= 0.0: return`). No XP is credited, no signals fire, the score screen sees only the XP accrued before death. This was originally documented (revision-0) as "XP credited for score screen" — that was design intent, not code reality. If post-death XP crediting is desired for the score screen, this requires a code change (remove the early-return condition and gate only the `level_reached` emit + Level Up panel open). Tracked as part of OQ-3 (revive scope).
...
**AC-12** **GIVEN** Player in `DEFEATED` state (`_is_dead == true`), **WHEN** `gain_experience(50)` is called, **THEN** the function early-returns at `player.gd:140-141` AND `current_xp` is unchanged AND no `experience_changed`, `level_reached`, or any other signal fires AND the score screen reports whatever `current_xp` was at the moment of death. (Code-true behavior per `_is_dead` guard. revision-0 of this GDD asserted XP would credit "for the score screen" — that was design intent not present in the code.)
